...
x = torch.empty(4,3)
print(x)
print(x.type()) #the values in tensors are float by defaut

#   Initialized tensors with .zeros() and .ones()
x = torch.zeros(4,3,dtype=torch.int32)#the values in the tensor are integers
print(x)
x = torch.ones(4,3)
print(x)
...
x = torch.rand(4,3)
print(x)
x = torch.randint(0,10,[4,3]) # min, max, size
print(x)
...
print(x.shape)
print(x.size())
print(x.device)# tensor run on CPU by default
