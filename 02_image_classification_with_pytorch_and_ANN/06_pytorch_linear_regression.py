...
class Model(nn.Module):
    def __init__(self,in_feature, out_feature): #in_feature, out_feature: size of input and output
        super().__init__()
...
for i in range (epochs):
    ...
    losses.append(loss)
    print(f'epoch:{i:2} loss: {loss.item():10.8f} weight:{model.linear.weight.item():10.8f}' f'bias:{model.linear.weight.item():10.8f}')
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
